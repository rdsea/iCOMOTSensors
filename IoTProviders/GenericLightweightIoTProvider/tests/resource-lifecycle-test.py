...
parser.add_argument('--gliot_provider', help='URL of the GLIOT Provider')
...
def single_test(max_workers,num_test):
    ...
    for i in range(num_test):
        if (dry_run):
            ...
            start = time.time()
            create_response=requests.post(create_resource,data=json.dumps(post_data),headers=headers)
            stop = time.time()
            creation_time =creation_time + stop -start
            creation_counter =creation_counter+1
            json_response = json.loads(create_response.text)
            gliotId=json_response['gliotId']
            get_data =list_resource+"/"+gliotId
            print(get_data)
            start = time.time()
            ...
            delete_data = delete_resource+"/"+gliotId
            print(delete_data)
            start = time.time()
    ...
    return np.array([creation_time/creation_counter,list_time/list_counter,deletion_time/deletion_counter])

##prepare for result file
with open(args.test_output, 'w',newline='',encoding='utf-8') as csvfile:
    fieldnames = ["nrclient","creationtime","listtime","deletiontime"]
