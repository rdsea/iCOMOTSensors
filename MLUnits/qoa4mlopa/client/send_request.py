...
client_info = {"id": "user1",
        "roles": "ml_user"}
service_info = {"id": "request", 
        "machinetypes": "small", 
        "metric": ["Accuracy", "ResponseTime"]}
metric = {"Accuracy": 90.0,
        "ResponseTime": 0.0,
        "Analytics": 60.0}
...
def publish_message(ml_client, file, qoa_client, qoa_url):
    f = open(file, 'r')
    ...
    for line in f:
        ...
        print(ml_response)
        metric["Accuracy"] = ml_response["Accuracy"]
        metric["ResponseTime"] = ml_response["ResponseTime"]
        qoa_response = qoa_client.qoa_report(metric, qoa_url)
...
if __name__ == '__main__':
    ...
    qoa_client = Qoa_Client(client_info,service_info)


    
    publish_message(ml_predition_client, args.file, qoa_client, qoa_url)
    time.sleep(1)
