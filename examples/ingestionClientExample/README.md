...
4. copy the contents of the `ingestionClient` folder into `src`
5. delete any `node_modules` folder in `src`
6. A `keyfile.json` must also be generated from the google cloud console, For further information read this: https://cloud.google.com/docs/authentication/production#obtaining_and_providing_service_account_credentials_manually
7. copy the keyfile.json into the `src/dataPlugins/bigQuery` folder
8. `config.bigQuery.yml` and `config.ingestionClient.yml` have already been preconfigured, feel free to change them if you have specific requirements
9. from this folder run `$ docker-compose up`
